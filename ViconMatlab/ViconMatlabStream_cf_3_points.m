...
clear all
%Subjects to track
SubjectstoTrack={'cf_dw'};%,'AlonTable'}; % names must match to vison system
% Program options
...
%                 Output_GetSegmentStaticRotationEulerXYZ.Rotation( 3 ) );
            ...
            fprintf( '        Global Translation: (%g, %g, %g) %s\n',               ...
             ...
             if strcmp('False',AdaptBool( Output_GetSegmentGlobalTranslation.Occluded) )
                crazyflie_pos = [crazyflie_pos;[Output_GetSegmentGlobalTranslation.Translation( 1 ),Output_GetSegmentGlobalTranslation.Translation( 2 ),Output_GetSegmentGlobalTranslation.Translation( 3 )]];
             end
        end
...
%                 AdaptBool( Output_GetSegmentGlobalRotationQuaternion.Occluded ) );
         ...
         if strcmp('False',AdaptBool( Output_GetSegmentGlobalRotationEulerXYZ.Occluded ) )
            cf_rotation = [cf_rotation;[Output_GetSegmentGlobalRotationEulerXYZ.Rotation( 1 ), Output_GetSegmentGlobalRotationEulerXYZ.Rotation( 2 ),Output_GetSegmentGlobalRotationEulerXYZ.Rotation( 3 )]];
        end
       end
        ...
        for MarkerIndex = 1
           ...
           Output_GetMarkerGlobalTranslation = MyClient.GetMarkerGlobalTranslation( SubjectName, MarkerName );
            ...
            fprintf( '      Marker #%d: %s (%g, %g, %g) %s\n',                     ...
             ...
             if strcmp('False',AdaptBool( Output_GetSegmentGlobalTranslation.Occluded) ) && strcmp('antena_marker', MarkerName)
                 antena_marker = [antena_marker; [Output_GetMarkerGlobalTranslation.Translation( 1 ),Output_GetMarkerGlobalTranslation.Translation( 2 ), Output_GetMarkerGlobalTranslation.Translation( 3 )]];
...
process_cf = 1
current_counter = load('measure_counter');
current_counter = current_counter.current_counter ;
if process_cf
offset =  [14,0,-15];
fixed = antena_marker + offset;
mean_pos = mean(fixed);
...
save(strcat('vicon_static_pos_changed_config_',string(current_counter)))
current_counter = current_counter+1;
save('measure_counter','current_counter')
end
